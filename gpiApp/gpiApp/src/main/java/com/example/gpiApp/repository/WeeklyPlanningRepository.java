...
public interface WeeklyPlanningRepository extends JpaRepository<WeeklyPlanning, Long> {
    ...
    List<WeeklyPlanning> findByUserId(@Param("userId") Long userId);

    @Query("SELECT wp FROM WeeklyPlanning wp WHERE wp.user.userId = :userId AND wp.weekNumber = :weekNumber AND wp.year = :year")
    Optional<WeeklyPlanning> findByUserAndWeek(@Param("userId") Long userId, @Param("weekNumber") Integer weekNumber, @Param("year") Integer year);

    @Query("SELECT wp FROM WeeklyPlanning wp WHERE wp.user.userId = :userId AND wp.complianceStatus = 'COMPLIANT'")
    List<WeeklyPlanning> findCompliantPlanningsByUser(@Param("userId") Long userId);

    @Query("SELECT wp FROM WeeklyPlanning wp WHERE wp.user.userId = :userId AND wp.isApproved = true")
    List<WeeklyPlanning> findApprovedPlanningsByUser(@Param("userId") Long userId);

    @Query("SELECT COUNT(wp) FROM WeeklyPlanning wp WHERE wp.user.userId = :userId AND wp.complianceStatus = 'COMPLIANT'")
    long countCompliantPlanningsByUser(@Param("userId") Long userId);
