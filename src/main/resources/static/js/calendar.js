...
// Event type colors (Google Calendar-like)
...
// Initialize FullCalendar with Google Calendar-like design
function initializeCalendar() {
    ...
    calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        select: function(info) {
            const startInput = document.getElementById('event-start') || document.getElementById('create-event-start-time');
            const endInput = document.getElementById('event-end') || document.getElementById('create-event-end-time');
...
async function loadCalendarEvents(startStr, endStr, successCallback, failureCallback) {
    try {
        ...
        const formattedEvents = events.map(event => ({
            ...
            backgroundColor: event.color || eventTypeColors[event.eventType] || '#4361ee',
            borderColor: event.color || eventTypeColors[event.eventType] || '#4361ee',
            textColor: '#ffffff',
    ...
    modal.innerHTML = `
        <div class="modal-content" style="max-width: 500px;">
            <div class="modal-header">
                ...
                <button class="close-btn" onclick="closeModal('event-detail-modal'); this.closest('.modal').remove();">×</button>
            </div>
            <div class="modal-body">
                ...
                ${event.extendedProps?.location ? `
                    <div style="margin-bottom: 16px;">
                        <strong>Location:</strong> ${escapeHtml(event.extendedProps.location)}
                    </div>
                ` : ''}
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" onclick="closeModal('event-detail-modal'); this.closest('.modal').remove();">Close</button>
...
async function handleCreateEvent(e) {
    ...
    try {
        await apiRequest('/api/calendar', 'POST', {
            ...
            startTime: formData.get('startTime') || formData.get('start'),
            endTime: formData.get('endTime') || formData.get('end'),
            color: formData.get('color') || '#4361ee',
            eventType: formData.get('eventType') || 'CUSTOM',
